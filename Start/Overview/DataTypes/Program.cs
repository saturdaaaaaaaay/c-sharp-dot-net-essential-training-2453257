...
Console.WriteLine($"{c + a}"); // char converted to integer value to complete expression
Console.WriteLine($"{(char)(c + a)}"); // cast integer value to a char

Console.WriteLine($"{f + a}"); // float + int = float
Console.WriteLine($"{f + c}"); // float + char = float
...
StructOp(s1); // passes a copy of s1
Console.WriteLine($"{s1.a}, {s1.b}"); // still has original s1 values because struct is value data type
...
ClassOp(cl); // operates on actual values
Console.WriteLine($"{cl.a}, {cl.b}"); // values have been changed by ClassOp because cl is a reference type
