...
void DoSomeBigOperation() {
    ...
    Console.WriteLine($"Allocated memory is: {GC.GetTotalMemory(false)}"); // GC = Garbage Collector
    Console.ReadLine();
}
...
GC.Collect(); // probably never have to actually use this since it's managed automatically
